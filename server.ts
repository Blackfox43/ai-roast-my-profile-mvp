...
async function generateRoast(rawInput: string, roastStyle: RoastStyle): Promise<RoastResult> {
  if (process.env.MOCK_AI === "true") {
    return {
      ...
      personaBreakdown: `This profile is giving ${roastStyle.replaceAll("_", " ")} energy with the confidence of a TED Talk and the substance of a loading screen. It wants to be iconic so badly it accidentally became a case study in over-branding.`,
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[AI Roast My Profile] Server listening on http://0.0.0.0:${PORT}`);
    console.log(`[AI Roast My Profile] Storage mode: ${store.mode}`);
  });
}
